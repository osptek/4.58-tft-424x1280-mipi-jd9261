...
esp_err_t esp_lcd_touch_new_i2c_jd9261(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
    ...
    esp_lcd_touch_handle_t jd9261 = (esp_lcd_touch_handle_t)calloc(1, sizeof(esp_lcd_touch_t));
    ESP_GOTO_ON_FALSE(jd9261, ESP_ERR_NO_MEM, err, TAG, "Touch handle malloc failed");

    /* Communication interface */
    jd9261->io = io;
    /* Only supported callbacks are set */
    jd9261->read_data = read_data;
    jd9261->get_xy = get_xy;
    jd9261->del = del;
    /* Mutex */
    jd9261->data.lock.owner = portMUX_FREE_VAL;
    /* Save config */
    memcpy(&jd9261->config, config, sizeof(esp_lcd_touch_config_t));
    ...
    if (config->int_gpio_num != GPIO_NUM_NC) {
        ...
        if (config->interrupt_callback) {
            esp_lcd_touch_register_interrupt_callback(jd9261, config->interrupt_callback);
        }
    }
    ...
    /* Reset controller */
    ESP_GOTO_ON_ERROR(reset(jd9261), err, TAG, "Reset failed");
    ESP_GOTO_ON_ERROR(read_fw_info(jd9261), err, TAG, "Read version failed");
